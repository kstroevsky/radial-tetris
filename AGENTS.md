...
- Treat mobile capsule artwork and touch geometry separately: preserve the near-edge Spin/Nudge visuals while providing generous invisible thumb targets and at least 48px pole targets. Nudge must act immediately, wait for a deliberate hold before repeating, and never carry a held repeat into the next piece.
- The phone controller is now intentionally buttonless and supersedes the capsule/rail artwork: the board itself owns tap-to-rotate-clockwise, tangential orbit, deliberate hold-to-soft-drop, and quick inward-swipe hard-drop gestures. Slow inward swipes must be ignored so they cannot leave a piece in an accidental state. Teach controls only through the first-use over-board guide and the small timed help control; keep normal gameplay feedback invisible.
- On phone layouts, passive text and decoration must be non-selectable, suppress the touch callout, and stay out of pointer hit-testing. Preserve pointer handling for real buttons, links, dialogs, and editable fields, and keep the document root scrollable for browser pull-to-refresh outside the board.
